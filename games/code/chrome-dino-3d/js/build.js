...
class EnemyManager {
	...
	makeNightReadable(materials) {
		const list = Array.isArray(materials) ? materials : [materials];
		list.forEach(material => {
			if(!material) return;
			// Keep cacti readable during the night cycle without making them glow loudly.
			material.emissive = new THREE.Color(0x274018);
	...
	async init() {
		...
		this.makeNightReadable(this.cache.cactus.material);
    ...
    increase_velocity(add = 1, init = false) {
        ...
        if(this.config.vel < 10) {
            ...
            player.setGravity(34);
                player.setGravity(27, true);

            logs.log('Speed level 1');
        }
        else if(this.config.vel >= 10 && this.config.vel < 20 && (player.jump.vel == 16 || init)) {
            player.setVelocity(20);
                player.setVelocity(1.1, true);

            player.setGravity(55);
                player.setGravity(36, true);
        ...
        else if(this.config.vel >= 20 && this.config.vel < 30 && (player.jump.vel == 20 || init)) {
            player.setVelocity(26);
                player.setVelocity(1.3, true);

            player.setGravity(92);
                player.setGravity(62, true);
        ...
        else if(this.config.vel >= 30 && (player.jump.vel == 26 || init)) {
            player.setVelocity(32);
                player.setVelocity(1.5, true);

            player.setGravity(138);
                player.setGravity(62, true);
...
function enableOmgDinoTestMode() {
  sessionStorage.setItem('omg-games-test-mode', 'true');
  window.__omgGamesTestMode = true;
  console.info('Dino test mode is on. This run will not be saved.');
  return 'Test mode on: this run will not be saved.';
}

Object.defineProperty(window, 'testMode', {
  configurable: true,
  get() {
    enableOmgDinoTestMode();
    return enableOmgDinoTestMode;
  }
});

window.setScore = function(points) {
  enableOmgDinoTestMode();

  const nextScore = Number.parseInt(points, 10);
  if(!Number.isFinite(nextScore) || nextScore < 0) {
    console.warn('Use setScore(123). The score must be a non-negative integer.');
    return false;
  }

  window.__omgDinoStartScore = nextScore;
  if(typeof score !== 'undefined' && score) {
    score.set(nextScore);
    score.last_flash_score = Math.trunc(nextScore);
  }

  console.info('Dino will start from score:', nextScore);
  return nextScore;
};
...
class EffectsManager {
    ...
    changeDaytime(daytime = 'day') {
      ...
      document.body.classList.toggle('is-night', !this.daytime.is_day);
...
class GameManager {
	...
	async start() {
        ...
        // init score
        score.set(Number.parseInt(window.__omgDinoStartScore || 0, 10) || 0);
